...
/**
 * \file SolverGodunovFiveEq.cpp
 *
 * Class SolverGodunovFiveEq implementation.
 */
...
#include <kalypsso/core/SmoothInterfaceFunctionData.h> // for get_max_ghostwidth

// Init conditions functors
#include <godunov_five_eq/init/init.h>
...
#include <godunov_five_eq/scheme/ComputeRefineFlags.h> // specific refinement strategies

// Utils functor
#include <kalypsso/core/ComputeSchlieren.h>

#include <godunov_five_eq/utils/ComputeDerivedQuantities.h>
#include <godunov_five_eq/utils/utils_aux.h>
...
// =======================================================
// =========== CLASS SolverGodunovFiveEq IMPL ============
// =======================================================

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
SolverGodunovFiveEq<dim, device_t>::SolverGodunovFiveEq(ParallelEnv const & par_env,
...
#endif // KALYPSSO_CORE_USE_HDF5
{
  ...
  // TODO: currently only one implementation available
  if (m_godunov_impl_version !=
      0 /*and m_godunov_impl_version != 1 and m_godunov_impl_version != 2*/)
  {
...
} // SolverGodunovFiveEq<dim, device_t>::SolverGodunovFiveEq
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::resize_auxiliary_data()
{
  ...
  assertm(m_godunov_implem != nullptr,
          "[SolverGodunovFiveEq::resize_auxiliary_data] godunov_implem pointer has invalid value.");
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::resize_solver_data()
{
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::update_mesh(bool reset_p4est_ghost)
{
...
} // SolverGodunovFiveEq<dim, device_t>::update_mesh

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::do_amr_cycle()
{
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::do_load_balancing()
{
...
template <size_t dim, typename device_t>
real_t
SolverGodunovFiveEq<dim, device_t>::compute_dt_local()
{
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::next_iteration_impl()
{
  ...
  if (m_mixing_monitor.should_record(m_iteration, m_t))
  {
    mixing_monitor_record();
  }
...
} // SolverGodunovFiveEq<dim, device_t>::godunov_unsplit

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::save_solution_impl(bool pure_checkpoint)
{

  KALYPSSO_PROFILING_REGION_HOST(m_profiling_mgr, IO);

  // if (m_params.output_exabrick_enabled)
  //   save_solution_exabrick();

  if (m_output_params.hdf5_enabled)
    save_solution_hdf5(pure_checkpoint);

} // SolverGodunovFiveEq<dim, device_t>::save_solution_impl()

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::print_monitoring_info()
{
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::print_monitoring_info_final()
{
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::register_volume_integrals(bool is_reference)
{
...
} // SolverGodunovFiveEq<dim, device_t>::register_volume_integrals

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::print_conservativity_check_report() const
{

  m_conservativity_check.print_report(m_par_env);

} // SolverGodunovFiveEq<dim, device_t>::print_conservativity_check_report

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
uint64_t
SolverGodunovFiveEq<dim, device_t>::total_mem_size_in_bytes()
{
  uint64_t total = 0;
  ...
  // actual writing (cell data)
  {
    ...
    if (!pure_checkpoint)
    {
      if (is_present(write_variables, std::string{ "rho_mix_schlieren" }))
      {
        ...
        total_num_bytes += m_hdf5_writer->write_quadrant_attribute(
          rho_mix_schlieren_h, 0, "rho_mix_schlieren", 0, local_num_quadrants);
      }
      ...
      for (DERIVED_QUANTITY derived_var : DERIVED_QUANTITY::_values())
      {
        ...
        // note "rho_mix" is already handled as a regular variable
        if (is_present(write_variables, name) and name != "rho_mix")
        {
    ...
    // see if godunov implementation has data to save
    total_num_bytes += m_godunov_implem->save_data(m_hdf5_writer);
...
} // SolverGodunovFiveEq<dim, device_t>::save_solution_hdf5
...
SolverGodunovFiveEq<dim, device_t>::fill_outside_quadrants(DataArrayBlock_t data)
{
  ...
  assertm(data.num_quadrants() == m_mesh_map->get_amr_mesh_info().local_num_quadrants_total(),
          "[SolverGodunovFiveEq::fill_outside_quadrants] Input array has the wrong size.");
...
} // SolverGodunovFiveEq<dim, device_t>::fill_outside_quadrants

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::mark_cells()
{
  ...
  auto const names = m_config_map.getStringVector(
    "amr", "variable_used_to_compute_refine_criterion", std::vector<std::string>{ "rho_mix" });

  for (auto const & name : names)
  {
    const auto maybe_derived_quantity = DERIVED_QUANTITY::_from_string_nocase_nothrow(name.c_str());
    if (*maybe_derived_quantity and name != "rho_mix")
    {
...
template <size_t dim, typename device_t>
void
SolverGodunovFiveEq<dim, device_t>::adapt_mesh()
{
...
template class SolverGodunovFiveEq<2, kalypsso::DefaultDevice>;
template class SolverGodunovFiveEq<3, kalypsso::DefaultDevice>;
