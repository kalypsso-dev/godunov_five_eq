...
/**
 * \file SolverGodunovFiveEq.h
 *
 * Class SolverGodunovFiveEq definition.
 *
 * Main class for solving two-material compressible hydrodynamics (5 equations model) using
 * finite volume MUSCL-Hancock scheme for 2D/3D.
 */
#ifndef KALYPSSO_GODUNOV_FIVE_EQ_SOLVER_GODUNOV_FIVE_EQ_H_
...
/**
 * Main data structure for 2D/3D MUSCL-Hancock scheme for block AMR.
 *
 * \tparam dim is dimension (integer: 2 or 3)
 * \tparam device_t is a kokkos device class (e.g. Kokkos::CudaSpace::device_type)
 */
template <size_t dim, typename device_t>
class SolverGodunovFiveEq : public kalypsso::SolverBase
{
...
public:
  ...
  // =================================================================================
  //! constructor
  SolverGodunovFiveEq(ParallelEnv const & par_env,
                      HydroParams const & params,
                      ConfigMap const &   config_map);
  //! destructor
  virtual ~SolverGodunovFiveEq();
  ...
  {
    SolverGodunovFiveEq<dim, device_t> * solver =
      new SolverGodunovFiveEq<dim, device_t>(par_env, params, config_map);
  ...
  solver_name() const override
  {
    if constexpr (dim == 2)
    {
      return "Godunov_FiveEq_2D";
    }
    else if constexpr (dim == 3)
    {
      return "Godunov_FiveEq_3D";
    }
...
public:
  ...
  SmoothInterfaceFunctionData<dim, device_t>
  smooth_interface_function_data()
  {
    return m_godunov_implem->m_sifd;
  }
...
private:
  ...
  //! integral mixing monitor
  IntegralMixingMonitor<dim, device_t> m_mixing_monitor;
...
extern template class SolverGodunovFiveEq<2, kalypsso::DefaultDevice>;
extern template class SolverGodunovFiveEq<3, kalypsso::DefaultDevice>;

} // namespace godunov_five_eq

} // namespace kalypsso

#endif // KALYPSSO_GODUNOV_FIVE_EQ_SOLVER_GODUNOV_FIVE_EQ_H_
