...
/**
 * \file BCShockBubble.cpp
 *
 * Implement inlet border conditions.
 *
 */
                                             ...
                                             ConfigMap const &          config_map)
{
  ...
  Kokkos::parallel_for(
    "FillOutsideShockBubble", Kokkos::RangePolicy<exec_space>(start, end), functor);
...
FillOutsideShockBubble<dim, device_t>::operator()(const index_t & i_global) const
{
  ...
  KOKKOS_ASSERT(is_valid_key && "[FillOutsideShockBubble] key_inside does not exist in hashmap !?");
  ...
  KOKKOS_ASSERT(
    ...
    "[FillOutsideShockBubble] i_oct_inside does not identify an octant inside domain ?!");
...
template class FillOutsideShockBubble<2, kalypsso::DefaultDevice>;
template class FillOutsideShockBubble<3, kalypsso::DefaultDevice>;
