...
void
InitCircleAdvectionDataFunctor<dim, device_t>::apply(
...
{
  // data init functor
  InitCircleAdvectionDataFunctor functor(
  ...
  Kokkos::parallel_for("kalypsso::godunov_five_eq::InitCircleAdvectionDataFunctor",
                       Kokkos::RangePolicy<exec_space>(0, nbCellsTotal),
                       functor);

} // InitCircleAdvectionDataFunctor::apply

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
KOKKOS_INLINE_FUNCTION void
InitCircleAdvectionDataFunctor<dim, device_t>::operator()(const int32_t & global_index) const
{
  ...
  auto const & circle_x = m_circle_advection_params.x;
  auto const & circle_y = m_circle_advection_params.y;
  auto const & circle_z = m_circle_advection_params.z;
  auto const & radius = m_circle_advection_params.r;
  auto const   radius2 = radius * radius;

  Kokkos::Array<real_t, dim> circle_center;
  circle_center[IX] = circle_x;
  circle_center[IY] = circle_y;
  if constexpr (dim == 3)
  {
    circle_center[IZ] = circle_z;
  }

  // determine if current is completely inside or outside of the circle
  bool is_inside = true;
  ...
  for (uint8_t i_corner = 0; i_corner < Corner::num_corners<dim>(); i_corner++)
  {
    ...
    auto r2 = (circle_x - xyz_corner[IX]) * (circle_x - xyz_corner[IX]) +
              (circle_y - xyz_corner[IY]) * (circle_y - xyz_corner[IY]);
    if constexpr (dim == 3)
      r2 += (circle_z - xyz_corner[IZ]) * (circle_z - xyz_corner[IZ]);
  ...
  if (is_inside)
  {
    ...
    m_Udata(cell_index, Hydro::IA1, iOct) = ONE_F - m_initial_states(0)[Hydro::IA0];
  ...
  else if (is_outside)
  {
    ...
    m_Udata(cell_index, Hydro::IA1, iOct) = ONE_F - m_initial_states(1)[Hydro::IA0];
  ...
  {
    // current cell intersect the circle border
    Kokkos::Array<real_t, dim> normal;
    real_t                     alpha;
    get_tangent_to_sphere(circle_center, radius, xyz, normal, alpha);

    // compute volume fraction of circle
    const auto vf =
      vof::compute_volume_fraction_of_rect_below_plane(normal, alpha, xyz - dx / 2, xyz + dx / 2);

    m_Udata(cell_index, Hydro::IA0, iOct) =
      vf * m_initial_states(0)[Hydro::IA0] + (ONE_F - vf) * m_initial_states(1)[Hydro::IA0];
    m_Udata(cell_index, Hydro::IA1, iOct) = ONE_F - m_Udata(cell_index, Hydro::IA0, iOct);

    m_Udata(cell_index, Hydro::IAD0, iOct) = vf * m_initial_states(0)[Hydro::ID];
    m_Udata(cell_index, Hydro::IAD1, iOct) = (ONE_F - vf) * m_initial_states(1)[Hydro::ID];
    m_Udata(cell_index, Hydro::ID, iOct) =
      m_Udata(cell_index, Hydro::IAD0, iOct) + m_Udata(cell_index, Hydro::IAD1, iOct);
    ...
    const auto rho_mixed = m_Udata(cell_index, Hydro::ID, iOct);
...
} // InitCircleAdvectionDataFunctor::operator ()

template class InitCircleAdvectionDataFunctor<2, kalypsso::DefaultDevice>;
template class InitCircleAdvectionDataFunctor<3, kalypsso::DefaultDevice>;

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
InitCircleAdvectionRefineFunctor<dim, device_t>::apply(
  DataArrayBlock_t const &             Udata,
  orchard_key_view_t<device_t> const & orchard_keys,
  ...
  ConfigMap const &                    config_map)
{
  ...
  InitCircleAdvectionRefineFunctor functor(
  ...
  {
    Kokkos::parallel_for("kalypsso::godunov_five_eq::InitCircleAdvectionRefineFunctor",
...
} // InitCircleAdvectionRefineFunctor::apply

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
KOKKOS_INLINE_FUNCTION void
InitCircleAdvectionRefineFunctor<dim, device_t>::operator()(TagRefineAlways const &,
                                                            const size_t & iOct) const
{
  m_amrflags(iOct) = AMRContextBase::KALYPSSO_DO_REFINE;
} // InitCircleAdvectionRefineFunctor::operator ()

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
KOKKOS_INLINE_FUNCTION void
InitCircleAdvectionRefineFunctor<dim, device_t>::operator()(TagRefineGeometric const &,
                                                            const size_t & iOct) const
{
  ...
  if (level == m_level_refine)
  {
    ...
    // compute distance to circle surface
    auto const & circle_x = m_circle_advection_params.x;
    auto const & circle_y = m_circle_advection_params.y;
    auto         d2 =
      (xyz[IX] - circle_x) * (xyz[IX] - circle_x) + (xyz[IY] - circle_y) * (xyz[IY] - circle_y);

    if constexpr (dim == 3)
    {
      auto const & circle_z = m_circle_advection_params.z;
      d2 += (xyz[IZ] - circle_z) * (xyz[IZ] - circle_z);
    }

    auto const & circle_radius = m_circle_advection_params.r;
    if (fabs(sqrt(d2) - circle_radius) < (block_length * KALYPSSO_NUM(1.25)))
    {
...
template class InitCircleAdvectionRefineFunctor<2, kalypsso::DefaultDevice>;
template class InitCircleAdvectionRefineFunctor<3, kalypsso::DefaultDevice>;

// =======================================================
// =======================================================
template <size_t dim, typename device_t>
void
InitCircleAdvection<dim, device_t>::apply(SolverGodunovFiveEq<dim, device_t> & solver)
{
  ...
  const auto nb_regions = InitCircleAdvectionDataFunctor<dim, device_t>::NB_REGIONS;
  ...
  const auto overwrite_velocity_field_type = core::get_overwrite_velocity_field_type(config_map);

  // first init of Udata
  InitCircleAdvectionDataFunctor<dim, device_t>::apply(solver.U(),
                                                       solver.mesh_map()->orchard_keys(),
                                                       solver.amr_mesh()->local_num_quadrants(),
                                                       initial_states,
                                                       config_map);
  core::OverwriteVelocityField<dim, device_t>::run(config_map,
                                                   solver.U(),
                                                   solver.amr_mesh()->local_num_quadrants(),
                                                   solver.mesh_map()->orchard_keys(),
...
} // InitCircleAdvection::apply

template class InitCircleAdvection<2, kalypsso::DefaultDevice>;
template class InitCircleAdvection<3, kalypsso::DefaultDevice>;
