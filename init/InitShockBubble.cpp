...
{
  const auto shock_bubble_params = ShockBubbleParams<device_t>(config_map);
  ...
  Kokkos::parallel_for("kalypsso::godunov_five_eq::InitShockBubbleDataFunctor",
  ...
  for (int ib = 0; ib < num_bubbles; ++ib)
  {
    ...
    if (bubble_vol_frac > ZERO_F)
    {
      ...
      m_Udata(cell_index, Hydro::IAD0, iOct) =
        phi0 * m_initial_states(1)[Hydro::IAD0] +
        (ONE_F - phi0) * m_initial_states(2 + ib)[Hydro::IAD0];

      m_Udata(cell_index, Hydro::IAD1, iOct) =
        phi0 * m_initial_states(1)[Hydro::IAD1] +
        (ONE_F - phi0) * m_initial_states(2 + ib)[Hydro::IAD1];

      m_Udata(cell_index, Hydro::ID, iOct) =
        m_Udata(cell_index, Hydro::IAD0, iOct) + m_Udata(cell_index, Hydro::IAD1, iOct);

      m_Udata(cell_index, Hydro::IA0, iOct) = phi0 * m_initial_states(1)[Hydro::IA0] +
                                              (ONE_F - phi0) * m_initial_states(2 + ib)[Hydro::IA0];

      m_Udata(cell_index, Hydro::IA1, iOct) = ONE_F - m_Udata(cell_index, Hydro::IA0, iOct);

      m_Udata(cell_index, Hydro::IU, iOct) = phi0 * m_initial_states(1)[Hydro::IU] +
                                             (ONE_F - phi0) * m_initial_states(2 + ib)[Hydro::IU];

      m_Udata(cell_index, Hydro::IV, iOct) = phi0 * m_initial_states(1)[Hydro::IV] +
                                             (ONE_F - phi0) * m_initial_states(2 + ib)[Hydro::IV];

      if constexpr (dim == 3)
      {
        m_Udata(cell_index, Hydro::IW, iOct) = phi0 * m_initial_states(1)[Hydro::IW] +
...
} // InitShockBubbleDataFunctor::operator ()
  ...
  ConfigMap const &                    config_map)
{
  ...
  {
    Kokkos::parallel_for("kalypsso::godunov_five_eq::InitShockBubbleRefineFunctor",
...
template <size_t dim, typename device_t>
KOKKOS_INLINE_FUNCTION void
InitShockBubbleRefineFunctor<dim, device_t>::operator()(TagRefineGeometric const &,
                                                        const size_t & iOct) const
{
  ...
  if (level == m_level_refine)
  {
    ...
    const auto d = fabs(xyz[IX] - m_sb_params.x_front);
    ...
    for (int ib = 0; ib < num_bubbles; ++ib)
    {
      ...
      auto const bubble_x = m_sb_params.bubble_x(ib);
      auto const bubble_y = m_sb_params.bubble_y(ib);
      auto       d2 =
        (xyz[IX] - bubble_x) * (xyz[IX] - bubble_x) + (xyz[IY] - bubble_y) * (xyz[IY] - bubble_y);

      if constexpr (dim == 3)
      {
        auto const bubble_z = m_sb_params.bubble_z(ib);
        d2 += (xyz[IZ] - bubble_z) * (xyz[IZ] - bubble_z);
      }

      auto const bubble_radius = m_sb_params.bubble_radius(ib);
      if ((fabs(xyz[IX] - m_sb_params.x_front) < block_length) or
...
template <size_t dim, typename device_t>
void
InitShockBubble<dim, device_t>::apply(SolverGodunovFiveEq<dim, device_t> & solver)
{
