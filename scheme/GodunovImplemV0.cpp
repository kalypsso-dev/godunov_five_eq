                                             ...
                                             real_t                   dt)
{
  ...
  this->read_fluxes_and_update_in_owned(U, U2, dt, IX);
  ...
  this->read_fluxes_and_update_in_owned(U, U2, dt, IY);
...
template <size_t dim, typename device_t>
uint64_t
GodunovImplemV0<dim, device_t>::save_data(
  [[maybe_unused]] std::shared_ptr<HDF5_Xdmf_Writer_t> hdf5_writer)
{
                                                                             ...
                                                                             int    direction)
{
  ...
  m_Fluxes.reshape(get_flux_block_sizes<dim>(m_Q_ghosted.block_size(), direction));
  m_u_star.reshape(get_flux_block_sizes<dim>(m_Q_ghosted.block_size(), direction));
                                                                ...
                                                                int                      direction)
{
  ...
  // check flux array sizes
  {
    [[maybe_unused]] const auto flux_block_sizes =
      get_flux_block_sizes<dim>(m_Q_ghosted.block_size(), direction);
